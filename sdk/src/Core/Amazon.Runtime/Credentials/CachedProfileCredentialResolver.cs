...
namespace Amazon.Runtime.Credentials
{
    ...
    internal class CachedProfileCredentialResolver : IDisposable
    {
        ...
        /// <list type="number">
        ///   <item><c>LastWriteTimeUtc</c> — provides immediate, synchronous detection on local file systems.</item>
        ///   <item><see cref="FileSystemWatcher"/> — provides reliable, push-based detection on network
        ///         file systems (SMB/CIFS) where <c>LastWriteTimeUtc</c> can return stale values due to
        ///         client-side metadata caching (<c>FileInfoCacheLifetime</c>).</item>
        /// </list>
        ...
        private sealed class ProfileCredentialSnapshot : IDisposable
        {
            ...
            /// <list type="bullet">
            ///   <item><c>FileSystemWatcher</c> callback (async, reliable on network storage)</item>
            ///   <item><c>LastWriteTimeUtc</c> comparison (sync, reliable on local file systems)</item>
            /// </list>
            /// </summary>
